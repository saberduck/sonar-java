...
public class ClassFieldCountCheck extends IssuableSubscriptionVisitor {
  ...
  public void visitNode(Tree tree) {
    ...
    if (fieldCount > threshold) {
      ...
      reportIssue(treeToReport(classTree), message);
    }
  }

  private Tree treeToReport(ClassTree classTree) {
    //simpleName can be null for anonymous classes
    return MoreObjects.firstNonNull(classTree.simpleName(), classTree.parent());
  }
